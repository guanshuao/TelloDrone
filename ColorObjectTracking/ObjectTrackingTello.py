...
def display(img):
    ...
    if trackingMode:
        cv2.putText(imgContour,"Tracking On",(frameWidth-200,50),cv2.FONT_HERSHEY_COMPLEX,1,(0,255,0),3)
    else:
        cv2.putText(imgContour,"Tracking Off",(frameWidth-200,50),cv2.FONT_HERSHEY_COMPLEX,1,(0,255,0),3)
...
while trackingMode:
    ...
    frame_read = cv2.cvtColor(frame_read, cv2.COLOR_BGR2RGB) # 将图像从BGR转换为RGB以便OpenCV可以处理它
    myFrame = frame_read
